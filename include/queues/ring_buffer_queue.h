...
private:
     ...
     * 
     * Each slot contains a flag indicating if data is present and the
     * actual data storage. The slot is aligned to cache line boundary
     * to prevent false sharing between adjacent slots.
     */
    struct Slot {
        alignas(CACHE_LINE_SIZE) std::atomic<bool> has_data;
        T data;
        
        /**
         * @brief Default constructor - creates an empty slot
         */
        ...
        Slot(Slot&& other) noexcept 
            : has_data(other.has_data.load(std::memory_order_relaxed))
            , data(std::move(other.data)) {
            other.has_data.store(false, std::memory_order_relaxed);
        }
        ...
        Slot& operator=(Slot&& other) noexcept {
            if (this != &other) {
                has_data.store(other.has_data.load(std::memory_order_relaxed),
...
public:
    ...
    explicit BoundedRingBufferMPMC(size_t capacity) 
        ...
        // Allocate buffer using reserve and emplace_back to avoid copying
    ...
    /**
     * @brief Get the number of slots (for debugging)
     * @return Total number of slots in the ring buffer
     */
    size_t slot_count() const { return buffer_.size(); }
};
